...
export default class RegisterStorageBuilderCompilerPass
{
    ...
    {
        let definition = container.getDefinition("kryptopus_candlestick_storage_builder_registry");

        let serviceIds = container.findTaggedServiceIds("kryptopus.candlestick_storage_builder");
        for (let serviceId of serviceIds) {
            ...
            for (let tag of storageTags) {
                if (tag.name !== "kryptopus.candlestick_storage_builder") {
                    continue;
                }

                if (!tag.builder_id) {
                    throw new Error(`Invalid tag "kryptopus.candlestick_storage_builder" for service ${serviceId}`);
                }
